...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    LENGTH_BIT_SIZE = 7;//provider.ConsumeIntegralInRange<size_t>(2, 63);
    ...
    size_t enc_ret_val = run_length_encode_v1(img_buf, width, height, rle_buf);

    uint8_t *decode_buf = (uint8_t *)malloc(img_buf_size);
    run_length_decode_v1(rle_buf, enc_ret_val, width, height, decode_buf);
